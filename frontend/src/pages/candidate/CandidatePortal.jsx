...
const CandidatePortal = () => {
  ...
  const handleRequestOTP = (e) => {
    e.preventDefault();
    // TODO: Call API to request OTP here
    setStep(2);
  };
  ...
  const handleConfirmAdmission = () => {
    if (window.confirm("Bạn có chắc chắn muốn xác nhận nhập học vào ngành Công nghệ thông tin?")) {
      // TODO: Call API to confirm admission
      setStep(4);
    }
  };
  ...
  const handleSubmitDocuments = () => {
    // TODO: Call API to upload documents
    alert("Gửi hồ sơ thành công! Cảm ơn bạn.");
  };
  ...
  const renderStep1 = () => (
    <div className="animate-in fade-in zoom-in duration-500">
      ...
      <form onSubmit={handleRequestOTP} className="space-y-6">
        ...
        <button 
          type="submit"
          className="w-full bg-red-700 hover:bg-red-800 text-white font-medium py-3.5 px-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center group"
        >
  ...
  const renderStep2 = () => (
    <div className="animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="text-center mb-8">
        ...
        <div className="bg-red-50 text-red-800 text-sm py-2 px-4 rounded-lg inline-flex items-center">
          <AlertCircle size={16} className="mr-2" />
          Mã OTP 6 số đã được gửi về email của bạn
        </div>
      </div>
      ...
      <form onSubmit={handleVerifyOTP} className="space-y-8">
        <div>
          ...
          <div className="text-center mt-3 text-sm text-gray-500">
            {countdown > 0 ? (
              ...
              <button type="button" className="text-red-600 font-medium hover:underline" onClick={() => setCountdown(59)}>
                Gửi lại mã OTP
              </button>
            )}
          </div>
        ...
        <div className="space-y-4">
          <button 
            type="submit"
            className="w-full bg-red-700 hover:bg-red-800 text-white font-medium py-3.5 px-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300"
          >
            Xác nhận OTP
          </button>
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="text-center mb-8">
          <h2 className={`text-3xl font-bold mb-2 ${isPassed ? 'text-gray-800' : 'text-red-600'}`}>
            {isPassed ? 'Chúc mừng, Nguyễn Văn A!' : 'Rất tiếc, Nguyễn Văn A!'}
          </h2>
          <p className="text-gray-500">
            {isPassed ? 'Dưới đây là kết quả xét tuyển của bạn' : 'Bạn chưa đủ điểm trúng tuyển vào ngành này đợt này'}
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 mb-6 relative overflow-hidden transition-all duration-500">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <h3 className="text-2xl font-bold text-red-700">Công Nghệ Thông Tin</h3>
            </div>
            {isPassed ? (
            ...
            <div className="animate-in fade-in slide-in-from-top-4 duration-500 mt-6">
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100 bg-white">
                    <tr className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 text-gray-800">Toán học</td>
                      <td className="px-4 py-3 text-right font-medium text-gray-800">{isPassed ? '9.20' : '8.20'}</td>
                    </tr>
                    <tr className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 text-gray-800">Vật lý</td>
                      <td className="px-4 py-3 text-right font-medium text-gray-800">{isPassed ? '8.75' : '7.75'}</td>
                    </tr>
                    <tr className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 text-gray-800">Hóa học</td>
                      <td className="px-4 py-3 text-right font-medium text-gray-800">{isPassed ? '9.00' : '8.00'}</td>
                    </tr>
                    <tr className="bg-gray-50/50">
                      <td className="px-4 py-3 text-gray-500 text-xs italic">Điểm ưu tiên (KV/ĐT)</td>
                      <td className="px-4 py-3 text-right font-medium text-gray-500 text-xs">0.30</td>
                    </tr>
                  </tbody>
                  <tfoot className="bg-red-50 border-t border-red-100">
                    <tr>
                      ...
                      <td className="px-4 py-3 text-right font-bold text-red-800 text-base">{isPassed ? '27.25' : '24.25'}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              {isPassed && (
                <button 
                  ...
                  className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-4 px-4 rounded-xl shadow-[0_4px_14px_0_rgba(22,163,74,0.39)] hover:shadow-[0_6px_20px_rgba(22,163,74,0.23)] hover:-translate-y-0.5 transition-all duration-300 animate-pulse relative overflow-hidden group"
                >
        ...
        >
          ...
          {isUploaded ? (
            <>
              ...
              <p className="text-sm font-medium text-green-700">{files[fileKey].name}</p>
    ...
    return (
      <div className="animate-in fade-in zoom-in-95 duration-500">
        ...
        <button 
          ...
          className={`w-full font-bold py-4 px-4 rounded-xl transition-all duration-300 shadow-md
            ${uploadedCount === 3 
            ...
            <button 
              onClick={() => {
                if(window.confirm("Bạn muốn quay lại từ đầu?")) {
                  setStep(1); setFormData({sbd:'', cccd:'', otp:''}); setFiles({cccd:null, hocBa:null, giayBao:null});
                }
              }}
